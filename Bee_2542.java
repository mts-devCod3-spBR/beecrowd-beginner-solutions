...
QUESTÃO
Iu-di-oh! é um jogo de cartas que virou uma verdadeira febre entre os jovens! Todo jogador de Iu-di-oh! tem seu próprio baralho, 
contendo várias cartas do jogo. Cada carta contém N atributos (como força, velocidade, inteligência, etc.). 
...
SAÍDA
Para cada caso de teste, imprima uma linha contendo “Marcos” se Marcos é o vencedor, “Leonardo” se Leonardo é o vencedor, 
ou “Empate” caso contrário (sem aspas).
...
public class Bee_2542 {
    public static void main(String[] args) {
        ...
        while (input.hasNext()) {
            int nAtributos = input.nextInt();
            int nCartasMarcos = input.nextInt();
            int nCartasLeonardo = input.nextInt();
            ...
            for (int i = 0; i < nCartasMarcos; i++) {
                for (int j = 0; j < nAtributos; j++) {
                    atributosCartasMarcos[i][j] = input.nextInt();
                }
            }
            for (int i = 0; i < nCartasLeonardo; i++) {
                for (int j = 0; j < nAtributos; j++) {
                    atributosCartasLeonardo[i][j] = input.nextInt();
                }
            }
            int cartaEscolhidaMarcos = input.nextInt() - 1;
            int cartaEscolhidaLeonardo = input.nextInt() - 1;
            int atributoSorteado = input.nextInt() - 1;
